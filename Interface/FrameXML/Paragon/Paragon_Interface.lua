...
-- ============================================================================
-- TOGGLE PARAGON FRAME
-- ============================================================================

--- Ouvre ou ferme le frame UIParagon
...
function UIParagon_OnLoad(self)
    ...
    -- Sync badge sur l'onglet CharacterFrame
    if ParagonCharacterTab_UpdateBadge then
        ParagonCharacterTab_UpdateBadge()
    end
end
...
-- ============================================================================
-- ONGLET PARAGON DANS LE CHARACTERFRAME
-- ============================================================================
-- Crée un onglet "Paragon" dans le CharacterFrame vanilla (comme Personnage/Rép./Comp.)
-- S'accroche automatiquement après le dernier onglet existant.

local function ParagonCharacterTab_Create()
    if not CharacterFrame then return end
    if _G["CharacterFrameTab_Paragon"] then return end
    ...
    -- -----------------------------------------------------------------------
    -- Créer l'onglet avec le template vanilla, mais avec un ID hors plage (99)
    -- PanelTemplates_SetNumTabs ne traite que les IDs 1..numTabs, donc notre
    -- onglet ID=99 ne sera jamais touché par le système de re-render vanilla.
    -- On appelle PanelTemplates_DeselectTab une seule fois pour fixer l'état visuel.
    -- -----------------------------------------------------------------------
    local tab = CreateFrame("Button", "CharacterFrameTab_Paragon", CharacterFrame, "CharacterFrameTabButtonTemplate")
    tab:SetID(99)  -- ID hors plage : ignoré par PanelTemplates_SetNumTabs
    tab:SetText("Paragon")

    -- Fixer l'état visuel "déselectionné" une fois pour toutes
    PanelTemplates_DeselectTab(tab)

    -- Ancrer après le dernier onglet existant
    ...
    -- Clic : ferme le CharacterFrame et ouvre l'UI Paragon
    ...
    tab:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText(L.PARAGON_TAB_TOOLTIP or "Paragon Anniversary", 1, 0.82, 0, 1)
        GameTooltip:AddLine(L.PARAGON_TAB_DESC or "Consultez et gérez vos statistiques Paragon et gagnez des récompenses.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    ...
    -- -----------------------------------------------------------------------
    -- Badge de notification dorée sur l'onglet (points non distribués)
    -- Parenté à UIParent, même texture que le MicroButton (UI-GroupLoot-Coin-Up)
    -- -----------------------------------------------------------------------

    local badge = CreateFrame("Button", "CharacterFrameTab_ParagonBadge", UIParent)
    badge:SetSize(18, 18)
    ...
    -- Pulsation du glow (tourne seulement quand visible)
    ...
    -- Tooltip identique au MicroButton Notification
    badge:SetScript("OnEnter", function(self)
        local L = GetLocaleTable and GetLocaleTable() or {}
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:SetText(L.NOTIFICATION_TITLE or "Points Paragon non distribués", 1, 0.82, 0, 1)
        GameTooltip:AddLine(L.NOTIFICATION_MESSAGE or "Vous avez des points Paragon non distribués !", 1, 1, 1, 1)
        GameTooltip:AddLine(" ")
        GameTooltip:AddLine(L.NOTIFICATION_DISMISS or "Cliquer pour masquer cette notification.", 0.5, 0.5, 0.5, 1)
        GameTooltip:Show()
    end)
    ...
    -- Frame de polling toujours active (indépendante du badge, OnUpdate tourne toujours)
    -- Vérifie toutes les secondes si les conditions sont réunies pour afficher le badge
    local poller = CreateFrame("Frame", "CharacterFrameTab_ParagonPoller", UIParent)
    poller.timer = 0
    poller:SetScript("OnUpdate", function(self, elapsed)
        ...
        local hasPoints = ParagonData and ParagonData.availablePoints and ParagonData.availablePoints > 0
        local charOpen  = CharacterFrame and CharacterFrame:IsShown()
        local onTab1    = charOpen and (PanelTemplates_GetSelectedTab(CharacterFrame) == 1)
        local uiClosed  = not (UIParagon and UIParagon:IsShown())

        if hasPoints and not badge.dismissed and uiClosed and onTab1 then
...
-- ============================================================================
-- Badge de notification sur l'onglet Paragon : mise à jour globale
-- ============================================================================

--- Met à jour la visibilité du badge de notification sur l'onglet Paragon.
-- Calqué sur ParagonMicroButton_UpdateNotification().
function ParagonCharacterTab_UpdateBadge()
    local tab = _G["CharacterFrameTab_Paragon"]
    ...
    local charFrameOpen = CharacterFrame and CharacterFrame:IsShown()
    local onTab1 = charFrameOpen and (PanelTemplates_GetSelectedTab(CharacterFrame) == 1)

    if hasUnspentPoints and not badge.dismissed and frameNotOpen and onTab1 then
        badge:ClearAllPoints()
        badge:SetPoint("TOPLEFT", tab, "TOPRIGHT", -20, 8)
        badge:Show()  -- OnUpdate prend le relais pour la pulsation et l'auto-hide
    else
        badge:Hide()
    end
end
...
-- ============================================================================
-- Visibilité de l'onglet Paragon : uniquement sur Tab1 (Personnage)
-- ============================================================================

local function ParagonTab_UpdateVisibility()
    local tab = _G["CharacterFrameTab_Paragon"]
    if not tab then return end

    -- PanelTemplates_GetSelectedTab retourne l'ID de l'onglet actif
...
-- Créer l'onglet au login (CharacterFrame est garanti chargé à ce stade)
local _paragonTabLoader = CreateFrame("Frame")
_paragonTabLoader:RegisterEvent("PLAYER_LOGIN")
_paragonTabLoader:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ParagonCharacterTab_Create()

        -- Hook sécurisé sur le clic des onglets vanilla du CharacterFrame
        -- CharacterFrameTab_OnClick est la fonction globale appelée par chaque onglet
        hooksecurefunc("CharacterFrameTab_OnClick", function()
            ParagonTab_UpdateVisibility()
        end)

        -- Hook sur CharacterFrame Show pour l'état à l'ouverture
        hooksecurefunc(CharacterFrame, "Show", function()
            ParagonTab_UpdateVisibility()
            ParagonCharacterTab_UpdateBadge()
        end)

        -- Quand UIParagon s'ouvre/ferme, mettre à jour le badge de l'onglet
        if UIParagon then
            hooksecurefunc(UIParagon, "Show", function()
                ParagonCharacterTab_UpdateBadge()
            end)
            hooksecurefunc(UIParagon, "Hide", function()
                ParagonCharacterTab_UpdateBadge()
            end)
        end
