...
DESTINY_SPELLBOOK_SPELL = {}
DESTINY_SPELLBOOK_SKILLLINE = {}
...
local shapeshiftIgnoreSpell = {9634, 1066, 768, 783, 40120, 2457, 71, 2458, 465, 643, 1032, 7294, 10290, 10291, 10292, 10293, 10298, 10299, 10300, 10301, 19746, 19876, 19888, 19891, 19895, 19896, 19897, 19898, 19899, 19900, 27149, 27150, 27151, 27152, 27153, 27733, 30515, 30519, 32223, 35126, 40803, 41106, 42184, 48263, 48265, 48266, 48941, 48942, 48943, 48945, 48947, 54043, 55357, 55366, 63510, 63514, 63531, 63611}
...
local spellbookMainProfessionData = {
	...
	[TRADESKILL_BLACKSMITHING] = {
		["Up"] = nil,
		["Down"] = {2018, 29844, 51300, 3538, 3100, 9785, 9788, 17039, 17040, 17041, 9787}
	},
	...
	[TRADESKILL_HERBALISM] = {
		["Up"] = nil,
		["Down"] = {2383, 2366, 2368, 3570, 11993, 28695, 50300}
	},
...
function SpellBookFrame_UpdateSpellRender()
	...
	for i = 1, GetNumSpellTabs() do
		...
		for id, name in pairs(DESTINY_SPELLBOOK_SKILLLINE) do
			if temp == name then
				lineID = id
			end
		end

		if DESTINY_SPELLBOOK_SPELL and DESTINY_SPELLBOOK_SPELL[lineID] and #DESTINY_SPELLBOOK_SPELL[lineID] > 0 then
			table.sort(DESTINY_SPELLBOOK_SPELL[lineID], function(a, b)
				return a.spellLevel < b.spellLevel
			end)

			for q = 1, #DESTINY_SPELLBOOK_SPELL[lineID] do
				local data = DESTINY_SPELLBOOK_SPELL[lineID][q]
				if SpellBookFrame.bookType ~= BOOKTYPE_PET then
	...
	for k, v in pairs(spellbookMainProfessionValue) do
		...
		if frame then
			...
			if v.downSpell then
				...
				_frame.professionName:SetText(v.skillName)
...
function SpellBookFrame_Update(showing)
	...
	SpellBookPrevPageButton:SetShown(needShow)
...
function SpellBookSkillLineTab_OnClick(self, id)
	...
	if tutorialSpellBook.spellLearn and id ~= 1 then
		HelpPlate_Hide(tutorialSpellBook.spellLearn)
	end
end
		...
		for i = 1, #split do
			if split[i] then
				...
				if skillLine and spellID and spellLevel then
					...
					if not DESTINY_SPELLBOOK_SPELL[skillLine] then
						DESTINY_SPELLBOOK_SPELL[skillLine] = {}
					end

					table.insert(DESTINY_SPELLBOOK_SPELL[skillLine], {spellID = tonumber(spellID), spellLevel = tonumber(spellLevel)})
				end
			end
		end
		SpellBookFrame_UpdateSpellRender()
		...
		for i = 1, #split do
			if split[i] then
				...
				if skillLineID and skillLineName then
					DESTINY_SPELLBOOK_SKILLLINE[tonumber(skillLineID)] = skillLineName
				end
			end
		end
		SpellBookFrame_UpdateSpellRender()
