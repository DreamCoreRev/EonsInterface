...
function ChatFrame_MessageEventHandler(self, event, ...)
	if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
		...
		if ( type == "LOOT" ) then
			if (string.find(arg1, string.sub(LOOT_ITEM_PUSHED_SELF, 1, #LOOT_ITEM_PUSHED_SELF-5)))
			or (string.find(arg1, string.sub(LOOT_ITEM_CREATED_SELF, 1, #LOOT_ITEM_CREATED_SELF-5)))
			or (string.find(arg1, string.sub(LOOT_ITEM_SELF, 1, #LOOT_ITEM_SELF-5))) then
				local s = strfind(arg1, "\124c");
				local _, e = strfind(arg1, "\124h\124c");
				local link = strsub(arg1, s, e);
				if ( link ) then
					LootWonAlertFrame_ShowAlert(link);
				end
			end

			self:AddMessage(arg1, info.r, info.g, info.b, info.id);
		elseif ( type == "SYSTEM" or type == "SKILL" or type == "MONEY" or type == "OPENING" or
				type == "TRADESKILLS" or type == "PET_INFO" or type == "TARGETICONS") then
			self:AddMessage(arg1, info.r, info.g, info.b, info.id);
