    ...
    if runeType then
        ...
        if runeTexture then
            runeTexture:SetTexture(
                'Interface\\AddOns\\PortraitFrameUI\\Textures\\PlayerFrame\\ClassOverlayDeathKnightRunes.BLP')
...
local function ReplaceBlizzardPlayerFrame(frame)
    ...
    restIconTexture:SetTexture("Interface\\AddOns\\PortraitFrameUI\\Textures\\PlayerFrame\\PlayerRestFlipbook.blp")
    ...
    for index = 1, MAX_COMBO_POINTS do
        ...
        for _, region in pairs { comboPoint:GetRegions() } do
            if region:GetObjectType() == 'Texture' and region:GetDrawLayer() == 'BACKGROUND' then
                ...
                region:SetTexture(
                    "Interface\\AddOns\\PortraitFrameUI\\Textures\\PlayerFrame\\ClassOverlayComboPoints.BLP")
        ...
        highlightTexture:SetTexture("Interface\\AddOns\\PortraitFrameUI\\Textures\\PlayerFrame\\ClassOverlayComboPoints.BLP")
...
local function ReplaceBlizzardTargetFrame(frame, target, isBoss)
    ...
    backgroundTexture:SetTexture("Interface\\AddOns\\PortraitFrameUI\\Textures\\TargetFrame\\NameBackground.blp")
	...
	if (target == FocusFrame) then
        local focusRaidTargetIconTexture = _G['FocusFrame' .. 'TextureFrame' .. 'RaidTargetIcon']
        focusRaidTargetIconTexture:ClearAllPoints()
        focusRaidTargetIconTexture:SetPoint("TOPRIGHT", -20, 18)
    end
