...
-- Résout l'icône d'un item via GetItemInfo.
-- Si le cache client n'a pas encore l'item, réessaie après 1s
-- et appelle onReady(iconPath) quand disponible.
local function GetItemIconPath(itemId, onReady)
  local fallback = "Interface\\Icons\\INV_Misc_QuestionMark"
  if not itemId then
    if onReady then onReady(fallback) end
    return fallback
  end
  local _, _, _, _, _, _, _, _, _, tex = GetItemInfo(itemId)
  if tex then
    if onReady then onReady(tex) end
    return tex
  end
  -- Cache manquant : réessaie dans 1 seconde
  if onReady then
    ...
    retryFrame:SetScript("OnUpdate", function(self, dt)
      ...
      local _, _, _, _, _, _, _, _, _, t = GetItemInfo(itemId)
      onReady(t or fallback)
    end)
  end
  return fallback
end
...
SlashCmdList["BMAH"] = function(msg)
  ...
  elseif msg == "fill" then
    ...
    -- show usage
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r Utilisation : /bmah [fill|flush] (GM SEULEMENT)")
  end
end
StaticPopupDialogs["BMAH_CONFIRM_BID"] = {
  text         = "Êtes-vous sûr de vouloir miser %dg sur |cff00ff00%s|r ?",
  button1      = ACCEPT,
  button2      = CANCEL,
  hideOnEscape = 1,
  timeout      = 0,
...
f:SetScript("OnEvent", function(self, event, ...)
  if event == "CHAT_MSG_SYSTEM" then
    local msg = ...
    if msg:match("%[BMAH%]|r Votre mise de %d+g a été acceptée") then
  ...
  elseif event == "CHAT_MSG_ADDON" then
    ...
    if prefix == ADDON_PREFIX then
      ...
      elseif cmd == "DONE" then
        ...
        if hotRow then
          GetItemIconPath(hotRow.itemId, function(tex)
            BlackMarketFrameHotItemIcon.texture:SetTexture(tex)
          end)
          BlackMarketFrameHotItemName:SetText(hotRow.itemName)
          BlackMarketFrameHotItemSubclass:SetText(hotRow.itemType)
          BlackMarketFrameSellerLine:SetText("Vendeur:\n"..hotRow.owner)
          BlackMarketFrameTimeLeftLine:SetText("Temps restant : "..hotRow.timeLeft)
        end
      end
    end
  end
  ...
  elseif prefix == DONE then
    ...
    if hotRow then
      ...
      if BlackMarketFrameHotItemName then
        BlackMarketFrameHotItemName:SetText(hotRow.itemName or "Article inconnu")
      end

      -- Subclass
      if BlackMarketFrameHotItemSubclass then
        BlackMarketFrameHotItemSubclass:SetText((hotRow.itemType or "Type inconnu"))
      end

      -- Seller
      if BlackMarketFrameSellerLine then
        BlackMarketFrameSellerLine:SetText("Vendeur:\n" .. (hotRow.owner or "inconnu"))
      end

      -- Time left
      if BlackMarketFrameTimeLeftLine then
        BlackMarketFrameTimeLeftLine:SetText("Temps restant : " .. (hotRow.timeLeft or "très long"))
      end
    end
...
function BlackMarketUI_UpdateList(data)
  ...
  for i = 1, #buttons do
    ...
    if record then
      if btn.Icon then
        local iconWidget = btn.Icon  -- capture la référence avant le callback async
        GetItemIconPath(record.itemId, function(tex)
          iconWidget:SetTexture(tex)
...
local function BMAHWhisperFilter(self, event, msg, author, ...)
  ...
  elseif event == "CHAT_MSG_WHISPER" then
    ...
    elseif tag == "BMAH_DONE" then
      -- all rows received
      DEFAULT_CHAT_FRAME:AddMessage("  → Transmission BMAH terminée.")
...
init:SetScript("OnEvent", function(self, event, addon)
  ...
  if not f then
    DEFAULT_CHAT_FRAME:AddMessage("BlackMarketFrame introuvable au chargement.")
    return
  end
...
init:SetScript("OnEvent", function(self, event)
  ...
  sellerLine:SetText("Marché actuellement fermé")
  ...
  timeLeftLine:SetText("Marché actuellement fermé")
  ...
  nameText:SetText("Marché actuellement fermé")
  ...
  subclassText:SetText("Marché actuellement fermé")
  ...
  do
    ...
    txt:SetText("Aucun article sur le marché pour l'instant. Revenez plus tard !")
    txt:Hide()
  ...
  do
    ...
    if frame then
      ...
      DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[BMAH UI]|r BlackMarketFrame introuvable !")
    end
  end
...
function BlackMarketBid_OnClick(self)
  ...
  if not selectedIndex then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r Veuillez d'abord sélectionner un article.")
    return
  end
  ...
  if not record then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r Sélection invalide.")
    return
  end
...
function BlackMarketHotBid_OnClick(self)
  if not hotItemIndex then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r Aucun article vedette disponible pour le moment.")
    return
  end
  local record = rows[hotItemIndex]
  if not record then
    DEFAULT_CHAT_FRAME:AddMessage("|cff69ccf0[BMAH]|r Données de l'article vedette perdues — veuillez rouvrir l'interface.")
    return
  end
